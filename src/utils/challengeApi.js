...
export const checkUserExists = async (email) => {
  try {
    const params = new URLSearchParams({ action: 'getProfile', email: email.toLowerCase().trim() });
...
export const checkUserByUsername = async (username) => {
  try {
    const params = new URLSearchParams({ action: 'getUserByUsername', username: username.trim() });
    const response = await fetch(`${SHEETS_URL}?${params}`);
    const data = await response.json();
    return {
      exists: data?.success === true,
...
// ── submitChallengeScore: NO 'no-cors' so we can read { winner, xpAwarded, completed }
