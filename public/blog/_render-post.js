(function() {
  ...
  if (!post) {
    root.innerHTML = '<div style="max-width:420px;margin:80px auto;text-align:center;padding:40px 24px;background:#fff;border-radius:16px;box-shadow:0 2px 16px rgba(108,63,201,0.10)"><div style="font-size:48px;margin-bottom:16px">😕</div><h2 style="font-size:20px;font-weight:800;color:#111827">Post not found</h2><p style="color:#6B7280;margin-top:8px">The post you are looking for does not exist.</p><a href="../index.html" style="display:inline-block;margin-top:20px;background:#6C3FC9;color:#fff;padding:10px 22px;border-radius:20px;font-weight:700;text-decoration:none">Back to Blog</a></div>';
    return;
  }

  if (!BlogEngine.isLive(post)) {
    root.innerHTML = '<div style="max-width:420px;margin:80px auto;text-align:center;padding:40px 24px;background:#fff;border-radius:16px;box-shadow:0 2px 16px rgba(108,63,201,0.10)"><div style="font-size:48px;margin-bottom:16px">⏳</div><h2 style="font-size:20px;font-weight:800;color:#111827">Coming Soon</h2><p style="color:#6B7280;margin-top:8px">This post publishes on ' + BlogEngine.formatDate(post.publishAt) + '. Check back then.</p><a href="../index.html" style="display:inline-block;margin-top:20px;background:#6C3FC9;color:#fff;padding:10px 22px;border-radius:20px;font-weight:700;text-decoration:none">Browse Live Posts</a></div>';
    return;
  }
  ...
  var PARTNER_AD = {
    ...
    desc: '30-day intensive programme with live tutors. Limited spots available.',
  ...
  if (otherPosts.length > 0) {
    relatedHtml = '<div class="blog-related"><div class="blog-related-title">More from EliteScholars</div><div class="blog-related-grid">' +
      otherPosts.map(function(rp) {
        ...
        return '<a href="' + href + '" class="blog-related-card"><span class="blog-related-cat" style="background:' + rs.bg + ';color:' + rs.color + '">' + rs.label + '</span><div class="blog-related-card-title">' + rp.title + '</div><div class="blog-related-meta">' + rp.readTime + ' min read &middot; ' + BlogEngine.formatDate(rp.publishAt) + '</div></a>';
      }).join('') +
    '</div></div>';
  }
    ...
    '<div class="blog-hero">' +
      '<div class="blog-container">' +
        ...
        '<div class="blog-meta">' +
          ...
          '<span class="blog-meta-dot">&middot;</span>' +
          '<span class="blog-meta-item">⏱ ' + post.readTime + ' min read</span>' +
        '</div>' +
      '</div>' +
    '</div>' +
    '<div class="blog-container"><div class="blog-body">' +
      ...
      '<div class="blog-share">' +
        '<div class="blog-share-title">Found this helpful? Share it with a friend who needs it 👇</div>' +
